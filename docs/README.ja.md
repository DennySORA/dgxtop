...
> NVIDIA DGXシステム向けの高性能インタラクティブシステムモニター。GPU、CPU、メモリ、ディスク容量/I/O、ネットワークをリアルタイムで監視。
...
**dgxtop**は、NVIDIA DGXインフラストラクチャ向けに設計された包括的なシステム監視ツールです。インタラクティブなターミナルUIを通じて、GPU使用率、VRAM、温度、消費電力、NVLinkトポロジ、ディスク使用量、システムリソースをリアルタイムで表示します。Rustで開発され、NVMLへの直接アクセスにより最高のパフォーマンスと信頼性を実現します。
...
- **完全なシステムビュー** — 単一のダッシュボードでCPUコアごとの使用率、メモリ（RAM + Swap）、ディスク容量（使用済み / 空き / 使用率）とI/O、ネットワークインターフェースを表示。
...
| **ディスク容量とI/O** | マウント済みファイルシステムの容量（使用済み / 空き / 使用率）、デバイスごとの読み書きスループット、IOPS、awaitレイテンシ |
...
- **デバイス選択** — `n/N`でネットワークインターフェース、`d/D`でディスクデバイスを切り替え。チャートと統計は選択中のデバイスに追従
...
**概要** — CPUゲージ、メモリバー、GPUカード、ディスク容量 + I/O、ネットワークパネルを備えたレスポンシブなダッシュボード。ディスクテーブルにはマウント済みファイルシステムの使用済み / 空き / 使用率が表示され、デバイスごとの読み書きスループット、IOPS、awaitレイテンシも継続して追跡されます。
